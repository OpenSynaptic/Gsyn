...
class _SendPageState extends ConsumerState<SendPage>
  ...
  void _sendIdRequest() =>
      _send('ID_REQUEST', PacketBuilder.buildIdRequest(_seq));
  void _sendTimeRequest() =>
      _send('TIME_REQUEST', PacketBuilder.buildTimeRequest(_seq));
  ...
  void _sendRawHex() {
    ...
    if (hex.isEmpty || hex.length % 2 != 0) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(const SnackBar(content: Text('无效 HEX 字符串（长度必须为偶数）')));
      return;
    }
    try {
          ...
          Expanded(
            child: TabBarView(
              ...
              children: [
                ...
                _RawTab(hexCtrl: _rawHexCtrl, onSend: _sendRawHex, log: _log),
              ],
            ),
          ),
        ],
                        ...
                        items: devices
                            .map(
                              (d) => DropdownMenuItem(
                                value: d.aid,
                                child: Text(
                                  '${d.name.isNotEmpty ? d.name : 'Device'} (AID: ${d.aid})',
...
class _ControlTab extends StatelessWidget {
  ...
  const _ControlTab({
    required this.seqCtrl,
    required this.assignAidCtrl,
    required this.onPing,
    required this.onPong,
    required this.onIdRequest,
    required this.onIdAssign,
    required this.onTimeRequest,
    required this.onHandshakeAck,
    required this.onHandshakeNack,
    required this.onSecureDictReady,
    required this.log,
  });
...
class _DataTab extends StatelessWidget {
  ...
  const _DataTab({
    ...
    required this.multiSensors,
    required this.onAddRow,
    required this.onSendSingle,
    required this.onSendMulti,
        ...
        Card(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                _CmdBadge(
                  '0x${OsCmd.dataFull.toRadixString(16).toUpperCase()}',
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 4),
                const Text(
                  '输入不含空格或带空格的十六进制字节串，例如：0901 02 03',
                  style: TextStyle(
                    fontSize: 11,
                    color: AppColors.textSecondary,
                  ),
