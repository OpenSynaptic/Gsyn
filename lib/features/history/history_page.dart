...
class _HistoryPageState extends ConsumerState<HistoryPage> {
  ...
  Future<void> _query() async {
    ...
    final d = await ref
        ...
        .query(fromMs: from, toMs: now, limit: 500);
    if (mounted) {
      setState(() {
        _data = d;
        _loading = false;
  ...
  Future<void> _export() async {
    ...
    final rows = [
      [l.csvTimestamp, l.csvDeviceAid, l.csvSensor, l.csvValue, l.csvUnit],
    ];
    for (final d in _data) {
      rows.add([
        DateTime.fromMillisecondsSinceEpoch(d.timestampMs).toIso8601String(),
        d.deviceAid.toString(),
        d.sensorId,
        d.value.toString(),
        d.unit,
      ]);
              ...
              itemBuilder: (ctx, i) {
                ...
                return Card(
                  child: ListTile(
                    dense: true,
                    title: Text(
                      '${d.sensorId}: ${d.value.toStringAsFixed(2)} ${d.unit}',
                    ),
                    subtitle: Text(
                      '${l.devicePrefix} ${d.deviceAid} • ${DateTime.fromMillisecondsSinceEpoch(d.timestampMs).toString().substring(0, 19)}',
