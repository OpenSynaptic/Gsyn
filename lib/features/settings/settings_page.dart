...
const _kTilePresets = <String, String>{
  ...
  'Stadia Alidade Dark'   : 'https://tiles.stadiamaps.com/tiles/alidade_smooth_dark/{z}/{x}/{y}.png',
  'Custom'                : '',
};
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          controller: _tab,
          tabs: [
            Tab(icon: const Icon(Icons.cable,        size: 18), text: l.settingsTabConnect),
            Tab(icon: const Icon(Icons.widgets,      size: 18), text: l.settingsTabCards),
            Tab(icon: const Icon(Icons.palette,      size: 18), text: l.settingsTabTheme),
            Tab(icon: const Icon(Icons.info_outline, size: 18), text: l.settingsTabInfo),
          ],
        ),
      ),
      body: TabBarView(
...
class _ConnectionTabState extends ConsumerState<_ConnectionTab> {
  ...
  @override
  void dispose() {
    _udpHostCtrl.dispose(); _udpPortCtrl.dispose();
    _mqttBrokerCtrl.dispose(); _mqttPortCtrl.dispose();
    _customTileCtrl.dispose(); _btAddressCtrl.dispose(); _btPortCtrl.dispose();
    super.dispose();
  }
    ...
    try {
      ...
      final labels = result.map((r) {
        switch (r) {
          ...
          default:                           return ref.read(appStringsProvider).noNetwork;
        }
      }).join('  ');
      if (mounted) {
        setState(() => _wifiStatus =
            labels.isEmpty ? ref.read(appStringsProvider).noNetwork : labels);
      }
    } catch (_) {
      if (mounted) setState(() => _wifiStatus = ref.read(appStringsProvider).detectFailed);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      AnimatedSize(
        ...
        child: _udpEnabled
            ? Column(children: [
                TextField(controller: _udpHostCtrl,
                    decoration: InputDecoration(
                        labelText: l.listenAddr, hintText: l.listenAddrHint)),
                const SizedBox(height: 8),
                TextField(controller: _udpPortCtrl,
      ...
      Text(
        l.tileCurrentUrl(_tilePreset == 'Custom'
            ? (_customTileCtrl.text.isEmpty ? '(未填写)' : _customTileCtrl.text)
            : _kTilePresets[_tilePreset]!),
      ...
      Card(
        ...
        child: Padding(
          ...
          child: Row(children: [
            const Icon(Icons.signal_wifi_statusbar_4_bar, size: 20, color: AppColors.info),
            const SizedBox(width: 10),
            Expanded(child: Text(_wifiStatus,
      ...
      SwitchListTile(
          ...
          subtitle: Text(l.btExperimentalHint,
              style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
          value: _btEnabled,
...
class _CardsTabState extends ConsumerState<_CardsTab> {
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      ...
      _toggle(l.cardDevicesLabel,    _cfg.kpiDevices,    (v) => _cfg = _cfg.copyWith(kpiDevices: v)),
      _toggle(l.cardOnlineRateLabel, _cfg.kpiOnlineRate, (v) => _cfg = _cfg.copyWith(kpiOnlineRate: v)),
      _toggle(l.cardAlertsLabel,     _cfg.kpiAlerts,     (v) => _cfg = _cfg.copyWith(kpiAlerts: v)),
      _toggle(l.cardThroughputLabel, _cfg.kpiThroughput, (v) => _cfg = _cfg.copyWith(kpiThroughput: v)),

      const Divider(),
      _SectionHeader(icon: Icons.show_chart, title: l.secLineChart),
      _toggle(l.toggleTempChart, _cfg.chartTemp,     (v) => _cfg = _cfg.copyWith(chartTemp: v)),
      _toggle(l.toggleHumChart,  _cfg.chartHumidity, (v) => _cfg = _cfg.copyWith(chartHumidity: v)),

      const Divider(),
      _SectionHeader(icon: Icons.speed, title: l.secGauges),
      _toggle(l.toggleGaugesRow1, _cfg.gaugesRow1, (v) => _cfg = _cfg.copyWith(gaugesRow1: v)),
      _toggle(l.toggleGaugesRow2, _cfg.gaugesRow2, (v) => _cfg = _cfg.copyWith(gaugesRow2: v)),

      const Divider(),
      _SectionHeader(icon: Icons.bar_chart, title: l.secCharts),
      _toggle(l.toggleBarChart, _cfg.barChart, (v) => _cfg = _cfg.copyWith(barChart: v)),
      _toggle(l.togglePieChart, _cfg.pieChart, (v) => _cfg = _cfg.copyWith(pieChart: v)),
            ...
            child: AnimatedContainer(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                Container(
                  width: 40, height: 40,
                  decoration: BoxDecoration(
                    color: p.seedColor, shape: BoxShape.circle,
                    boxShadow: sel
                        ? [BoxShadow(color: p.seedColor.withValues(alpha: 0.5), blurRadius: 8)]
                        : null,
                  ),
...
class _InfoTabState extends ConsumerState<_InfoTab> {
  ...
  Future<void> _load() async {
    ...
    if (mounted) setState(() { _devices = devs; _dbSizeKb = sz ~/ 1024; });
  }
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(context, true), child: Text(l.confirm)),
        ],
      ),
    );
    if (confirm == true) {
      ...
      child: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        stats.when(
          data: (s) => Column(children: [
            _InfoRow(l.infoUdpStatus,  s.udpConnected  ? l.connectedBadge : l.disconnectedBadge),
            _InfoRow(l.infoMqttStatus, s.mqttConnected ? l.connectedBadge : l.disconnectedBadge),
        ...
        // ── Known devices / AIDs ──────────────────────────────────────────────
        _SectionHeader(icon: Icons.devices, title: l.knownDevicesCount(_devices.length)),
        if (_devices.isEmpty)
          Padding(
              padding: const EdgeInsets.symmetric(vertical: 12),
              child: Text(l.noDeviceData, style: const TextStyle(color: AppColors.textSecondary)))
        else
          ..._devices.map((d) {
            ...
            return ListTile(
              ...
              title: Text(d.name.isNotEmpty ? d.name : '${l.devicePrefix} ${d.aid}',
                  style: const TextStyle(fontSize: 13)),
              subtitle: Text(
            ...
            child: ClipRRect(
              ...
              child: Stack(children: [
                ...
                if (sel)
                  Positioned(
                      top: 4, right: 4,
                      child: Icon(Icons.check_circle, size: 16, color: checkColor)),
              ]),
            ),
          ),
        );
...
class _LangButton extends StatelessWidget {
  ...
  const _LangButton({required this.label, required this.icon, required this.selected, required this.onTap});
