    ...
    return Scaffold(
      ...
      body: Column(children: [
        Padding(padding: const EdgeInsets.all(8), child: TextField(
          decoration: InputDecoration(hintText: l.searchHint, prefixIcon: const Icon(Icons.search, size: 20), isDense: true),
...
class _DeviceDetailState extends ConsumerState<DeviceDetailPage> {
  ...
  Widget build(BuildContext context) {
    final l = ref.watch(appStringsProvider);
    return Scaffold(
      appBar: AppBar(title: Text(widget.device.name.isNotEmpty ? widget.device.name : '${l.devicePrefix} ${widget.device.aid}')),
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        Text(l.latestReadings, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600)),
        ...
        Card(child: Padding(padding: const EdgeInsets.all(16), child: Row(
          ...
          children: [
            Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: ElevatedButton.icon(onPressed: () {}, icon: const Icon(Icons.network_ping), label: const Text('PING')))),
            Expanded(child: Padding(padding: const EdgeInsets.symmetric(horizontal: 4), child: ElevatedButton.icon(onPressed: () {}, icon: const Icon(Icons.restart_alt), label: const Text('RESET')))),
          ],
        ))),
      ]),
    );
