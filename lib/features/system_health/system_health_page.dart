    ...
    return Scaffold(
      ...
      body: stats.when(
        data: (s) => SingleChildScrollView(padding: const EdgeInsets.all(8), child: Column(children: [
          GridView.count(crossAxisCount: 2, shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), childAspectRatio: 1.4, children: [
            KpiCard(title: l.udpStatus,   value: s.udpConnected  ? l.statusConnected : l.statusDisconnected, icon: Icons.cable, color: s.udpConnected  ? AppColors.online : AppColors.offline),
            KpiCard(title: l.mqttStatus,  value: s.mqttConnected ? l.statusConnected : l.statusDisconnected, icon: Icons.cloud, color: s.mqttConnected ? AppColors.online : AppColors.offline),
            KpiCard(title: l.messagesPerSec, value: '${s.messagesPerSecond}', icon: Icons.speed,   color: AppColors.info),
            KpiCard(title: l.totalMessages,  value: '${s.totalMessages}',     icon: Icons.message, color: AppColors.primary),
          ]),
          KpiCard(title: l.dbSize, value: '$_dbSizeKb KB', icon: Icons.storage, color: AppColors.secondary),
        ])),
